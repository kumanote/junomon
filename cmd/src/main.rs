...
#[structopt(about = "junomon running options")]
struct Opts {
...
fn main() {
    ...
    debug!(
        "Loaded juno daemon monitoring tool config, config: {:?}",
        config
    );

    // set up juno client pool
    let mut client_pool = HashMap::new();
    for checker in &config.checkers {
        let endpoint = checker.juno_grpc_endpoint();
        info!("this tool will connect to {}", endpoint.as_str());
        let client = JunodClient::new(endpoint.clone());
        client_pool.insert(endpoint, Arc::new(Mutex::new(client)));
    }
    junocli::set_client_pool(client_pool);

    if let Err(err) = junomon::start() {
        error!("{}", err);
    }
