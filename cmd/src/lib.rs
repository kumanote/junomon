...
pub fn start() -> Result<()> {
    ...
    for checker in &app_config.checkers {
        ...
        let runtime = manager
            .junod_endpoint(checker.juno_grpc_endpoint().as_str())
...
impl CheckManager {
    pub fn junod_endpoint(&mut self, junod_endpoint: &str) -> &mut Self {
        self.junod_endpoint = junod_endpoint.to_owned();
        self
    }
    ...
    fn setup(&mut self) -> tokio::runtime::Runtime {
        let runtime = tokio::runtime::Builder::new_multi_thread()
            .thread_name(format!("{}", self.junod_endpoint.as_str()))
            .enable_all()
            .build()
            .expect("Failed to create tokio runtime!");
        if self.check_if_syncing {
            ...
            let checker =
                checker::is_syncing::IsSyncingChecker::new(self.junod_endpoint.clone(), receiver);
    ...
    async fn next(&mut self) -> Result<&mut Self> {
        ...
        let latest_block_response = junocli::get_client(&self.junod_endpoint)
            .lock()
            .await
            .fetch_latest_block()
            .await?;
        ...
        if from_height <= latest_height {
            for height in from_height..=latest_height {
                let block_message: BlockMessage = if height == latest_height {
                    ...
                    let block_response = junocli::get_client(&self.junod_endpoint)
                        .lock()
                        .await
