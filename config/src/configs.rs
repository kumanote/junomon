...
pub struct CheckerConfig {
    pub juno_grpc_scheme: String,
    pub juno_grpc_host: String,
    pub juno_grpc_port: u16,
...
impl CheckerConfig {
    pub fn juno_grpc_endpoint(&self) -> String {
        format!(
            "{}://{}:{}",
            self.juno_grpc_scheme, self.juno_grpc_host, self.juno_grpc_port
        )
    }
}
...
impl SelfValidation for CheckerConfig {
    fn validate(&self) -> Result<()> {
        if self.juno_grpc_scheme != "http" && self.juno_grpc_scheme != "https" {
            return Err(anyhow!(
                "juno daemon grpc_scheme must be either 'http' or 'https'..."
            ));
        }
...
impl FromEnv for CheckerConfig {
    fn from_env() -> Result<Self> {
        Ok(Self {
            juno_grpc_scheme: "http".to_owned(),
            juno_grpc_host: "127.0.0.1".to_owned(),
            juno_grpc_port: 9090,
...
impl TryFrom<CheckerToml> for CheckerConfig {
    ...
    fn try_from(toml: CheckerToml) -> Result<Self> {
        ...
        if let Some(juno_grpc_scheme) = toml.juno_grpc_scheme {
            result.juno_grpc_scheme = juno_grpc_scheme;
        }
        if let Some(juno_grpc_host) = toml.juno_grpc_host {
            result.juno_grpc_host = juno_grpc_host;
        }
        if let Some(juno_grpc_port) = toml.juno_grpc_port {
            result.juno_grpc_port = juno_grpc_port;
        }
