...
FROM alpine:3.16
COPY --from=builder /junomon/target/release/junomon /usr/local/bin/junomon
RUN chmod +x /usr/local/bin/junomon
...
RUN addgroup -g ${GROUP_ID} junomon; \
    adduser -D -u ${USER_ID} -G junomon -h /junomon -s "/bin/bash" junomon;

# install su-exec
RUN apk add --no-cache su-exec; \
    su-exec junomon true;
