...
impl IsSyncingChecker {
    pub fn new(junod_endpoint: String, receiver: Receiver<IsSyncingMessage>) -> Self {
        Self {
            junod_endpoint,
            receiver,
        }
        ...
        while let Some(message) = self.receiver.next().await {
            match message {
                IsSyncingMessage::Check => {
                    match junocli::get_client(&self.junod_endpoint)
                        .lock()
                        .await
                        .fetch_syncing()
                        .await
                    {
                        Ok(syncing) => {
                            if syncing {
                                error!(
                                    "the juno daemon: {} is syncing",
                                    self.junod_endpoint.as_str()
                                );
                            } else {
                                info!(
                                    "the juno daemon: {} is synced",
                                    self.junod_endpoint.as_str()
                                );
                            }
                        }
