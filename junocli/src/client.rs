...
use anyhow::anyhow;
use anyhow::Context;
use juno_grpc_client as proto;
use juno_grpc_client::tonic::{self, transport::Channel, Code};
use logger::prelude::*;

#[derive(Debug, Clone)]
pub struct JunodClient {
    endpoint: String,
...
impl JunodClient {
    pub async fn establish_connection(endpoint: &str) -> Result<Channel> {
        tonic::transport::Endpoint::new(endpoint.to_owned())?
            .connect()
            .await
            .with_context(|| format!("failed to connect juno daemon grpc endpoint: {}", endpoint))
    }
